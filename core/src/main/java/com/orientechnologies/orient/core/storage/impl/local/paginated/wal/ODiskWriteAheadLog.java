...
public class ODiskWriteAheadLog extends OAbstractWriteAheadLog {
  ...
  public ODiskWriteAheadLog(int maxPagesCacheSize, int commitDelay, long maxSegmentSize, final String walPath,
    ...
    try {
      ...
      if (walFiles.length == 0) {
        LogSegment logSegment = new LogSegment(this, new File(this.walLocation, getSegmentName(0)), maxPagesCacheSize);
        logSegment.init();
        ...
        for (File walFile : walFiles) {
          LogSegment logSegment = new LogSegment(this, walFile, maxPagesCacheSize);
          logSegment.init();
    ...
    try {
      ...
      if (last.filledUpTo() >= maxSegmentSize) {
        ...
        if (record instanceof OAtomicUnitEndRecord && activeOperations.size() == 1 || (!(record instanceof OOperationUnitRecord)
          ...
          last = new LogSegment(this, new File(walLocation, getSegmentName(last.getOrder() + 1)), maxPagesCacheSize);
          last.init();
          last.startFlush();
  ...
  public void moveLsnAfter(OLogSequenceNumber lsn) throws IOException {
    ...
    try {
      ...
      last = new LogSegment(this, new File(walLocation, getSegmentName(lsn.getSegment() + 1)), maxPagesCacheSize);
      last.init();
      last.startFlush();
